...
const House: FC<HouseProps> = ({ filterOpen }) => {
  ...
  const { data, isLoading, error } = useGetAllHousesQuery({
    ...
    propertyType,
    furnishing,
    carParking: carParking.toString(),
  ...
  if (error) {
    ...
    return <NotFound text={message} statusCode={status} backTo="/" />;
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full">
      ...
      {data?.houses.length === 0 ? (
        <NotFound text={"We couldn't find any houses for your criteria."} statusCode={404} />
      ) : (
        <div className="w-full sm:w-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-8 ">
